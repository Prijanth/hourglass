...
/* ── ICONS ──────────────────────────────────────────────────────────── */
export const Icon = ({ name, size = 18, stroke = "currentColor", strokeWidth = 1.7, fill = "none" }) => {
  const paths = {
...
/* ── LOGO ───────────────────────────────────────────────────────────── */
export const Logo = ({ size = "md", showWord = true, light = false }) => {
    ...
    <div style={{ display: "inline-flex", alignItems: "center", gap: 10, color: light ? "#FAFAF7" : "#0F1320" }}>
      <div style={{
        ...
        <svg width={dim * 0.6} height={dim * 0.6} viewBox="0 0 24 24" fill="none">
          <path d="M5 2h14M5 22h14" stroke="#00C9A7" strokeWidth="2" strokeLinecap="round"/>
          <path d="M6 3c0 5 12 5 12 9s-12 4-12 9" stroke="#00C9A7" strokeWidth="2" strokeLinecap="round" fill="none"/>
          <path d="M18 3c0 5-12 5-12 9s12 4 12 9" stroke="#00C9A7" strokeWidth="2" strokeLinecap="round" fill="none"/>
          <circle cx="12" cy="13" r="1.6" fill="#00C9A7"/>
        </svg>
      </div>
      {showWord && (
...
/* ── AVATAR ─────────────────────────────────────────────────────────── */
const AVATAR_COLORS = [
  ["#FFD9C2","#A03A0E"], ["#D8E9FF","#1B3B7A"], ["#E1F5DB","#2A6B1F"],
  ["#F4D9F7","#5E1671"], ["#FFE5C2","#7A3A05"], ["#D9EFEC","#0A5A4B"],
  ["#FFDFE3","#931E2E"], ["#E6E2FF","#2B1F7A"],
]
...
/* ── PILL ───────────────────────────────────────────────────────────── */
export const Pill = ({ children, tone = "default", live = false, style }) => {
  const cls = { default: "hg-pill", teal: "hg-pill hg-pill-teal", coral: "hg-pill hg-pill-coral", ink: "hg-pill hg-pill-ink", amber: "hg-pill hg-pill-amber" }[tone]
  return (
...
/* ── SLOT INDICATOR ─────────────────────────────────────────────────── */
export const SlotIndicator = ({ filled, total, size = 8, gap = 4 }) => (
  <div style={{ display: "inline-flex", gap }}>
    {Array.from({ length: total }).map((_, i) => (
      <div key={i} style={{ width: size * 2.5, height: size, borderRadius: 999, background: i < filled ? "var(--ink)" : "var(--line-strong)" }}/>
    ))}
  </div>
)
...
/* ── PLACEHOLDER ────────────────────────────────────────────────────── */
export const Placeholder = ({ label, height = 120, style = {} }) => (
  <div className="hg-placeholder" style={{ height, ...style }}>{label}</div>
)

/* ── BG GRID (grille décorative pour les fonds sombres) ─────────────── */
export const BgGrid = () => {
