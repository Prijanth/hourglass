...
import { Icon, Logo, Avatar, Pill, BgGrid } from './ui'
...
const LandingPage = ({ navigate = () => {} }) => {
  ...
  const [submitted, setDone]  = React.useState(false)
  ...
  const handleWaitlist = async (e) => {
    ...
    try {
      await fetch('/', {
        ...
        body: new URLSearchParams({ 'form-name': 'waitlist', email, role }).toString(),
      })
    } finally {
      setDone(true)
      setSub(false)
  ...
  return (
    <div className="hg-app" style={{ width: "100%", background: "var(--bg)" }}>
      {/* NAV */}
      <header style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button className="hg-btn hg-btn-primary" onClick={() => navigate('waitlist')}>
            Rejoindre la liste <Icon name="arrowRight" size={14}/>
          </button>
        </div>
      </header>
      ...
      <section style={{ padding: "88px 64px 80px", position: "relative" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 64, alignItems: "center", maxWidth: 1312 }}>
          <div>
            ...
            <p style={{ fontSize: 21, color: "var(--ink-muted)", marginTop: 28, maxWidth: 560, lineHeight: 1.5 }}>
              Hourglass, c'est le recrutement façon speed dating. Les recruteurs ouvrent un créneau en direct, les candidats prennent la file, et les deux côtés s'engagent dans une vraie conversation chronométrée.
            </p>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 36, color: "var(--ink-muted)", fontSize: 13 }}>
              <div style={{ display: "flex" }}>
                {["Sophie Marin","Akira Tan","Léa Voss","Marc Hale"].map((n, i) => (
                  <div key={n} style={{ marginLeft: i === 0 ? 0 : -10 }}><Avatar name={n} size="sm"/></div>
                ))}
              </div>
        ...
        <div style={{ display: "flex", gap: 48, alignItems: "center", whiteSpace: "nowrap" }}>
          {["Product Designer senior · 4 places","Ingénieur Backend · 2 places","Responsable Marketing · live dans 14 min","Data Scientist · session complète","Développeur iOS · 5 places","UX Researcher · live dans 2h","Growth PM · 3 places","Ingénieur Frontend · en direct","Customer Success Lead · 5 places"].map((t, i) => (
            <span key={i} style={{ fontSize: 13, color: "var(--ink-muted)", display: "inline-flex", alignItems: "center", gap: 8 }}>
              <span className="hg-dot-live" style={{ background: i % 3 === 0 ? "var(--coral)" : "var(--teal)" }}/>{t}
            </span>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "120px 64px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, position: "relative", maxWidth: 1312 }}>
          <div style={{ position: "absolute", left: "50%", top: 0, bottom: 0, width: 1, background: "var(--line)", transform: "translateX(-50%)" }}/>
          <FlowColumn title="Côté candidat" color="teal" icon="user" steps={[
            { n: "01", t: "Déposez votre CV", d: "Glisser, déposer, c'est fait. Réutilisable pour chaque session." },
            { n: "02", t: "Candidatez à un créneau live", d: "Parcourez les recruteurs qui ont ouvert une session cette semaine." },
            { n: "03", t: "Suivez votre position", d: "File d'attente en temps réel — vous êtes n°3, pas perdu dans une boîte noire." },
            { n: "04", t: "Décrochez l'entretien de 30 min", d: "Si le recruteur vous accepte dans ses 5 minutes, vous filez direct sur un créneau visio." },
          ]}/>
          <FlowColumn title="Côté recruteur" color="ink" icon="briefcase" steps={[
            { n: "01", t: "Ouvrez une session", d: "Choisissez une date, une heure et un nombre de candidats." },
            { n: "02", t: "Sprintez en 5 minutes", d: "Un candidat, un CV, un chrono. Fini de scroller 200 profils à 23h." },
            { n: "03", t: "Accepter ou passer", d: "Deux boutons. La décision avance avec vous." },
            { n: "04", t: "Se rencontrer, vraiment", d: "Les candidats acceptés tombent sur un créneau visio de 30 min dans votre agenda." },
          ]}/>
        </div>
      </section>
      ...
      <section style={{ padding: "0 64px 120px" }}>
        <div style={{ background: "var(--ink)", color: "var(--bg)", borderRadius: 28, padding: "80px 72px", position: "relative", overflow: "hidden", maxWidth: 1312 }}>
          <BgGrid/>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, position: "relative" }}>
            <DiffCard kind="old" title="Le recrutement aujourd'hui" rows={[
            ...
            <DiffCard kind="new" title="Le recrutement sur Hourglass" rows={[
      ...
      {/* STATS */}
      <section style={{ padding: "0 64px 120px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)", maxWidth: 1312 }}>
          {[
            ...
            <div key={i} style={{ padding: "48px 32px", borderLeft: i === 0 ? "none" : "1px solid var(--line)" }}>
      ...
      {/* WAITLIST CTA */}
      <section style={{ padding: "0 64px 120px" }}>
        <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 28, padding: 64, display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 64, alignItems: "center", maxWidth: 1312 }}>
          <div>
            <Pill tone="coral" live>Liste ouverte</Pill>
            <h2 className="hg-display" style={{ fontSize: 52, marginTop: 20, lineHeight: 1.02 }}>Soyez dans la première salle.</h2>
            <p style={{ color: "var(--ink-muted)", fontSize: 17, marginTop: 16, maxWidth: 480 }}>
              Nous ouvrons 50 places candidats et 10 places recruteurs par semaine.
            </p>
          </div>
          <div>
            {submitted ? (
              <div style={{ display: "flex", alignItems: "center", gap: 14, padding: 24, background: "var(--teal-tint)", border: "1px solid rgba(0,201,167,0.3)", borderRadius: 16 }}>
                <div style={{ width: 40, height: 40, borderRadius: 999, background: "var(--teal)", display: "grid", placeItems: "center", flexShrink: 0 }}>
                  <Icon name="check" size={20} stroke="var(--ink)" strokeWidth={2.5}/>
                </div>
                <div>
                  <div style={{ fontWeight: 600, fontSize: 16 }}>Vous êtes dans la file !</div>
                  <div style={{ fontSize: 13, color: "var(--teal-deep)", marginTop: 4 }}>On vous envoie un mail dès qu'un créneau correspond.</div>
                </div>
              </div>
            ) : (
              <form onSubmit={handleWaitlist}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input type="email" name="email" required className="hg-input" placeholder="vous@boulot.com" value={email} onChange={e => setEmail(e.target.value)} style={{ flex: 1 }}/>
                  <button type="submit" className="hg-btn hg-btn-lg hg-btn-teal" disabled={submitting}>
  ...
  <div style={{ position: "relative", height: 540 }}>
    <div style={{
      ...
      <div style={{ textAlign: "center", padding: "8px 0 24px" }}>
        <div className="hg-mono" style={{ fontSize: 88, fontWeight: 500, letterSpacing: "-0.03em", color: "var(--teal)" }}>3:47</div>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
        <button style={{ ...bigBtn, background: "rgba(255,255,255,0.06)", color: "var(--bg)" }}><Icon name="x" size={18}/> Passer</button>
        <button style={{ ...bigBtn, background: "var(--teal)", color: "var(--ink)" }}><Icon name="check" size={18}/> Accepter</button>
      </div>
    </div>
    <div style={{
      ...
      <div style={{ display: "flex", gap: 4 }}>
        {[1,2,3,4,5].map(n => (
          <div key={n} style={{ flex: 1, height: 6, borderRadius: 999, background: n <= 2 ? "var(--ink)" : n === 3 ? "var(--teal)" : "var(--line-strong)" }}/>
        ))}
      </div>
    </div>
  </div>
...
const FlowColumn = ({ title, color, icon, steps }) => (
  <div style={{ padding: "8px 32px" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 40 }}>
      <div style={{ width: 40, height: 40, borderRadius: 12, background: color === "teal" ? "var(--teal)" : "var(--ink)", color: color === "teal" ? "var(--ink)" : "var(--bg)", display: "grid", placeItems: "center" }}>
...
const DiffCard = ({ kind, title, rows }) => (
  <div style={{
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
      {kind === "old"
        ? <Icon name="x" size={16}/>
        : <div style={{ width: 18, height: 18, borderRadius: 999, background: "var(--teal)", display: "grid", placeItems: "center" }}><Icon name="check" size={12} stroke="var(--ink)" strokeWidth={2.4}/></div>
      }
      <span style={{ fontWeight: 500, fontSize: 14, color: kind === "new" ? "var(--teal)" : "rgba(250,250,247,0.7)" }}>{title}</span>
    </div>
    <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12 }}>
      {rows.map((r, i) => (
        <li key={i} style={{ fontSize: 16, color: kind === "old" ? "rgba(250,250,247,0.55)" : "var(--bg)", textDecoration: kind === "old" ? "line-through" : "none", textDecorationColor: "rgba(255,92,92,0.5)" }}>{r}</li>
      ))}
    </ul>
  </div>
)
