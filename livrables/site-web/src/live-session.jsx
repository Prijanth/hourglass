...
import { Icon, Logo, Avatar, Pill, SlotIndicator, fmtTime, BgGrid } from './ui'
...
export const CountdownRing = ({ seconds, total, color }) => {
  const r = 44
  const c = 2 * Math.PI * r
...
const LiveSession = () => {
  ...
  const [secondsLeft, setSec] = React.useState(REVIEW_SECONDS)
  const [running, setRunning] = React.useState(true)
  const [outcomes, setOut]    = React.useState({})
  ...
  const decide = (kind) => {
    setOut(o => ({ ...o, [candidate.id]: kind }))
    setConfirm(null)
    if (idx + 1 < SESSION_CANDIDATES.length) { setIdx(idx + 1); setSec(REVIEW_SECONDS) }
    else setRunning(false)
  }

  return (
    <div className="hg-app" style={{ width: "100%", minHeight: "100vh", background: "var(--ink)", color: "var(--bg)", display: "grid", gridTemplateRows: "auto 1fr auto", position: "relative" }}>
      <BgGrid/>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 28px", borderBottom: "1px solid rgba(255,255,255,0.08)", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "rgba(250,250,247,0.6)" }}>15:{String(13 + idx * 5).padStart(2, "0")}</span>
          <button onClick={() => setRunning(r => !r)} style={iconBtnDark}><Icon name={running ? "pause" : "play"} size={14}/></button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr 360px", minHeight: 0, position: "relative", zIndex: 1 }}>
        ...
        <aside style={{ padding: "24px 20px", borderRight: "1px solid rgba(255,255,255,0.08)", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {SESSION_CANDIDATES.map((c, i) => {
              ...
              return (
                <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: 12, borderRadius: 12, background: isCurrent ? "rgba(0,201,167,0.1)" : "transparent", border: isCurrent ? "1px solid rgba(0,201,167,0.3)" : "1px solid transparent", opacity: out ? 0.55 : 1 }}>
          ...
          <div style={{ marginTop: 24, padding: 14, borderRadius: 12, background: "rgba(255,255,255,0.04)" }}>
            ...
            {[["Lus",`${reviewed}/${SESSION_CANDIDATES.length}`,"var(--bg)"],["Acceptés",`${Object.values(outcomes).filter(o=>o==="accept").length}`,"var(--teal)"],["Passés",`${Object.values(outcomes).filter(o=>o==="pass").length}`,"#FF8B8B"]].map(([l,v,c])=>(
              <div key={l} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginTop: 6 }}>
                <span style={{ fontSize: 13 }}>{l}</span>
                <span className="hg-mono" style={{ fontSize: 14, fontWeight: 500, color: c }}>{v}</span>
              </div>
            ))}
          </div>
        </aside>
        ...
        <section style={{ padding: "28px 36px", display: "grid", gridTemplateRows: "auto 1fr", gap: 24, minHeight: 0 }}>
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", gap: 28, alignItems: "center" }}>
            ...
            <div>
              <div className="hg-eyebrow" style={{ color: "rgba(250,250,247,0.5)", marginBottom: 6 }}>
                Candidat {idx + 1} sur {SESSION_CANDIDATES.length} · {Math.max(0, SESSION_CANDIDATES.length - reviewed - 1)} après celui-ci
              </div>
              <div className="hg-display" style={{ fontSize: 34, lineHeight: 1.05 }}>{candidate.name}</div>
              <div style={{ color: "rgba(250,250,247,0.6)", fontSize: 14, marginTop: 4 }}>{candidate.current} · {candidate.loc} · {candidate.yrs} ans d'expérience</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div className="hg-eyebrow" style={{ color: "rgba(250,250,247,0.5)", marginBottom: 4 }}>Chrono de revue</div>
              <div className="hg-mono" style={{ fontSize: 56, fontWeight: 500, color: ringColor, letterSpacing: "-0.03em", lineHeight: 1 }}>{fmtTime(secondsLeft)}</div>
          ...
          {/* CV Preview */}
          <div style={{ background: "var(--bg)", color: "var(--ink)", borderRadius: 16, overflow: "hidden", boxShadow: "0 30px 60px -20px rgba(0,0,0,0.4)" }}>
        ...
        {/* RIGHT SIDEBAR */}
        <aside style={{ padding: "24px", borderLeft: "1px solid rgba(255,255,255,0.08)", overflowY: "auto", display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[["Localisation",candidate.loc],["Expérience",`${candidate.yrs} ans`],["Prétentions",candidate.salary],["Mode de travail",candidate.remote],["Préavis",`${candidate.noticeDays} jours`],["Langues",candidate.languages.join(", ")]].map(([l,v])=>(
                <div key={l} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: 12 }}>
                  <span style={{ color: "rgba(250,250,247,0.5)" }}>{l}</span>
                  <span style={{ color: "var(--bg)", fontWeight: 500, textAlign: "right" }}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <textarea placeholder="Noter quelque chose pour l'entretien…" style={{ width: "100%", minHeight: 90, resize: "none", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", color: "var(--bg)", borderRadius: 10, padding: 12, fontSize: 13, fontFamily: "var(--font-body)", outline: "none" }} defaultValue={idx === 0 ? "Bon travail design system chez Mirror — demander la stratégie de migration." : ""}/>
          </div>
          <div style={{ height: 1, background: "rgba(255,255,255,0.08)" }}/>
      ...
      <footer style={{ padding: "20px 36px", borderTop: "1px solid rgba(255,255,255,0.08)", display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 16, alignItems: "center", position: "relative", zIndex: 1, background: "rgba(15,19,32,0.6)", backdropFilter: "blur(8px)" }}>
        <button onClick={() => decide("pass")} style={{ ...decisionBtn, background: "rgba(255,92,92,0.10)", color: "#FF8B8B", border: "1px solid rgba(255,92,92,0.25)" }}>
          <Icon name="x" size={22}/>
          <div style={{ textAlign: "left" }}>
            <div style={{ fontSize: 18, fontWeight: 600 }}>Passer</div>
            <div style={{ fontSize: 11, opacity: 0.7 }}>Envoyer un refus bienveillant → suivant</div>
          </div>
          <kbd style={kbd}>P</kbd>
        </button>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "0 16px" }}>
          ...
          {secondsLeft === 0 && <Pill tone="coral" live>Décidez maintenant</Pill>}
        </div>
        <button onClick={() => decide("accept")} style={{ ...decisionBtn, background: "var(--teal)", color: "var(--ink)", border: "1px solid var(--teal)", justifyContent: "flex-end" }}>
          <kbd style={{ ...kbd, background: "rgba(15,19,32,0.15)", color: "var(--ink)" }}>A</kbd>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 18, fontWeight: 600 }}>Accepter</div>
            <div style={{ fontSize: 11, opacity: 0.7 }}>Réserver auto un entretien de 30 min</div>
          </div>
...
const CVPreview = ({ c }) => (
  <div style={{ padding: "32px 40px", overflow: "hidden", position: "relative" }}>
    <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 36 }}>
      <div>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <p style={{ fontSize: 13, lineHeight: 1.6, color: "var(--ink)", margin: 0 }}>
            Plus de {c.yrs} ans à designer du produit. Actuellement {c.current}, je livre du travail à l'intersection de {c.tags[0].toLowerCase()} et {c.tags[1]?.toLowerCase() || "du craft"}.
          </p>
        </div>
        <div style={{ marginTop: 24 }}>
            ...
            {[
              ...
              <div key={i} style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--ink-muted)", marginTop: 3 }}>Pilotage du design web + mobile.</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ borderLeft: "1px solid var(--line)", paddingLeft: 28 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 22 }}>
          {c.tags.map(t => <div key={t} style={{ fontSize: 13, padding: "6px 10px", background: "var(--surface-2)", borderRadius: 6 }}>{t}</div>)}
        </div>
        ...
        <div style={{ fontSize: 13, textDecoration: "underline", textDecorationColor: "var(--line-strong)" }}>{c.name.toLowerCase().split(" ")[0]}.work</div>
      </div>
    </div>
...
const iconBtnDark  = { width: 32, height: 32, borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "var(--bg)", cursor: "pointer", display: "inline-grid", placeItems: "center" }
const iconBtnLight = { width: 28, height: 28, borderRadius: 6, border: "1px solid var(--line)", background: "var(--bg)", color: "var(--ink-muted)", cursor: "pointer", display: "inline-grid", placeItems: "center" }
