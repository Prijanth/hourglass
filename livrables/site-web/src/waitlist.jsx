...
import { CountdownRing } from './live-session'
  ...
  return (
    <div className="hg-app" style={{
      ...
      <div style={{ padding: "56px 72px", display: "flex", flexDirection: "column" }}>
        <div style={{ cursor: "pointer" }} onClick={() => navigate('accueil')}>
          <Logo size="md"/>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", maxWidth: 540 }}>
          ...
          <p style={{ fontSize: 18, color: "var(--ink-muted)", marginTop: 22, lineHeight: 1.55 }}>
            Sessions live. Revues de 5 minutes. Entretiens de 30 minutes.
            Nous ouvrons <strong style={{ color: "var(--ink)" }}>50 places candidats</strong> et{" "}
            <strong style={{ color: "var(--ink)" }}>10 équipes RH</strong> par semaine.
          </p>
          ...
          <div style={{ marginTop: 40, padding: 24, background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 20, boxShadow: "var(--shadow-card)" }}>
            {submitted ? (
              <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "8px 0" }}>
                <div style={{ width: 48, height: 48, borderRadius: 999, background: "var(--teal)", display: "grid", placeItems: "center", flexShrink: 0 }}>
                  <Icon name="check" size={22} stroke="var(--ink)" strokeWidth={2.5}/>
                </div>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, fontFamily: "var(--font-display)" }}>Vous êtes dans la file !</div>
                  <div style={{ fontSize: 14, color: "var(--ink-muted)", marginTop: 4 }}>
                    On vous envoie un mail dès qu'un créneau correspond. Promis.
                  </div>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                ...
                <input type="email" name="email" required className="hg-input" placeholder="vous@boulot.com" value={email} onChange={e=>setEmail(e.target.value)}/>

                <label style={{ ...fieldLabel, marginTop: 14 }}>
                  {role === "candidat" ? "Je cherche" : "Je recrute pour"}
                </label>
                <input name="search" className="hg-input"
                  placeholder={role === "candidat" ? "ex. Product Designer senior, remote, Europe" : "ex. Ingénieur backend · Berlin · 2 postes"}
                  value={reason} onChange={e=>setReason(e.target.value)}/>

                <button type="submit" className="hg-btn hg-btn-xl hg-btn-teal" disabled={submitting} style={{ width: "100%", marginTop: 18, justifyContent: "center" }}>
                  {submitting ? "Envoi…" : "Réserver ma place"} {!submitting && <Icon name="arrowRight" size={16}/>}
                </button>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 14, fontSize: 11, color: "var(--ink-faint)" }}>
                  ...
                  <span>
                    <span className="hg-dot-live" style={{ display: "inline-block", marginRight: 6, background: "var(--teal)" }}/>
                    <strong style={{ color: "var(--ink)" }}>2 847</strong> déjà dans la file
                  </span>
                </div>
              </form>
            )}
      ...
      {/* RIGHT — dark panel */}
      <div style={{ background: "var(--ink)", color: "var(--bg)", padding: "56px", position: "relative", overflow: "hidden" }}>
        <BgGrid/>
...
const fieldLabel = { display: "block", fontSize: 12, fontWeight: 500, color: "var(--ink-muted)", marginBottom: 8 }
const featureCard = { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 22 }
