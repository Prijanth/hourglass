...
import { Icon, Logo, Avatar, Pill, SlotIndicator, BgGrid } from './ui'

const CANDIDATE_NAME  = "Marin Lavoie"
const CANDIDATE_EMAIL = "marin.lavoie@gmail.com"

export const SidebarLink = ({ icon, label, active, badge }) => (
  <div style={{
    display: "flex", alignItems: "center", gap: 12,
    padding: "10px 14px", borderRadius: 10,
    color: active ? "var(--ink)" : "var(--ink-muted)",
    background: active ? "var(--surface-2)" : "transparent",
    fontWeight: active ? 500 : 450, fontSize: 14, cursor: "pointer", position: "relative",
  }}>
...
export const AppShell = ({ children, role = "candidate", page = "Sessions", navigate = () => {} }) => {
  ...
  return (
    <div className="hg-app" style={{ width: "100%", minHeight: "100vh", background: "var(--bg)", display: "grid", gridTemplateColumns: "240px 1fr" }}>
      <aside style={{ background: "var(--surface)", borderRight: "1px solid var(--line)", padding: "24px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
...
export const TopBar = ({ title, sub, children }) => (
  <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 36px", borderBottom: "1px solid var(--line)", background: "var(--bg)" }}>
    <div>
...
const SESSIONS = [
  { id: "s1", co: "Northwind Robotics", role: "Product Designer senior", date: "jeu. 28 mai", time: "15:00 — 16:00", filled: 3, total: 5, location: "Remote · Europe", status: "live",     joined: false },
  { id: "s2", co: "Helix Bio",          role: "Biologiste computationnel", date: "ven. 29 mai", time: "10:00 — 11:30", filled: 4, total: 6, location: "Boston · Hybride", status: "upcoming", joined: true,  queue: 3 },
  { id: "s3", co: "Loom & Lattice",     role: "Designer de marque",         date: "lun. 2 juin",  time: "14:00 — 15:00", filled: 2, total: 5, location: "Remote · Monde", status: "upcoming", joined: false },
  { id: "s4", co: "Atlas Fret",         role: "Responsable logistique",     date: "mar. 3 juin",  time: "11:00 — 12:30", filled: 5, total: 5, location: "Hambourg",       status: "full",     joined: false },
  { id: "s5", co: "Quartz Analytics",   role: "Data Scientist Staff",        date: "mer. 4 juin",  time: "16:00 — 17:00", filled: 2, total: 5, location: "Remote · Amériques", status: "upcoming", joined: false },
  { id: "s6", co: "Postscript",         role: "Growth PM",                   date: "jeu. 5 juin",  time: "09:30 — 10:30", filled: 1, total: 4, location: "Londres",        status: "upcoming", joined: false },
]

export const SessionCard = ({ s }) => {
  const [joined, setJoined] = React.useState(s.joined || false)
  const statusPill = {
  ...
  return (
    <div className="hg-card" style={{ padding: 22, display: "flex", flexDirection: "column", gap: 14, cursor: "pointer" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {joined
          ? <Pill tone="teal"><Icon name="check" size={11}/> Dans la file · n°{s.queue || 1}</Pill>
          : s.status === "full"
            ? <button className="hg-btn hg-btn-ghost" disabled style={{ opacity: 0.5 }}>Complet</button>
            : <button className="hg-btn" style={{ background: "var(--ink)", color: "var(--bg)", padding: "8px 14px" }} onClick={() => setJoined(true)}>
                Candidater <Icon name="arrowRight" size={12}/>
              </button>
        }
      </div>
    </div>
            ...
            {[
              ...
              <div key={i} style={{ padding: 14, borderRadius: 12, background: s.current ? "rgba(0,201,167,0.12)" : "rgba(255,255,255,0.04)", border: s.current ? "1px solid var(--teal)" : "1px solid rgba(255,255,255,0.08)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                  <div style={{ width: 14, height: 14, borderRadius: 999, background: s.done ? "var(--teal)" : "rgba(255,255,255,0.15)", display: "grid", placeItems: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {["Cette semaine","Semaine prochaine","Toutes"].map((f, i) => (
                <button key={f} style={{ padding: "6px 14px", borderRadius: 999, border: "none", background: i === 0 ? "var(--ink)" : "transparent", color: i === 0 ? "var(--bg)" : "var(--ink-muted)", fontSize: 12, cursor: "pointer", fontWeight: 500 }}>{f}</button>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20, position: "sticky", top: 24 }}>
        <div className="hg-card" style={{ padding: 22 }}>
          ...
          <div style={{ marginTop: 14, padding: 12, background: "var(--teal-tint)", borderRadius: 10, display: "flex", alignItems: "flex-start", gap: 10, fontSize: 12, color: "var(--teal-deep)" }}>
            ...
            <span><strong>CV solide.</strong> Les recruteurs passent en moyenne <strong>4:12</strong> sur le vôtre.</span>
          </div>
        </div>
