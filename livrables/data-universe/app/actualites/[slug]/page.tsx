...
function renderInline(text: string): React.ReactNode[] {
  ...
  while (remaining.length > 0) {
    ...
    const linkPos = linkMatch?.index ?? Infinity;
    if (boldPos === Infinity && codePos === Infinity && linkPos === Infinity) {
      parts.push(<span key={idx++}>{remaining}</span>);
      break;
    }
    const minPos = Math.min(boldPos, codePos, linkPos);
    if (minPos === boldPos && boldMatch) {
      ...
      if (linkPos > 0) parts.push(<span key={idx++}>{remaining.slice(0, linkPos)}</span>);
      parts.push(
        <a key={idx++} href={linkMatch[2]} target="_blank" rel="noopener noreferrer"
          style={{ color: "var(--indigo)", fontWeight: 600, textDecoration: "underline", textUnderlineOffset: 3 }}>
          {linkMatch[1]}
        </a>
      );
...
function renderBlock(block: string, i: number, isInSources: boolean) {
  if (block.startsWith("## ")) {
    ...
    if (label === "Sources") {
      return (
        <h2 key={i} id={id} style={{
          ...
          marginTop: 56, marginBottom: 12, letterSpacing: "0.08em",
  ...
  if (isInSources) {
    return (
      <p key={i} style={{ marginBottom: 8, lineHeight: 1.65, color: "#64748B", fontSize: 13.5, padding: "8px 12px", borderRadius: 8, background: "#F8FAFC", border: "1px solid #E2E8F0" }}>
        {renderInline(block)}
      </p>
    );
  }
    ...
    <>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 24px", display: "grid", gridTemplateColumns: "1fr 280px", gap: 56, alignItems: "start" }}>
        <article style={{ fontSize: 16.5, minWidth: 0 }}>
          {blocks.map((block, i) => {
            const sourcesIdx = blocks.findIndex(b => b.startsWith("## Sources"));
            const inSources = sourcesIdx !== -1 && i > sourcesIdx;
            return renderBlock(block, i, inSources);
          })}
        ...
        <ArticleSidebar headings={headings} related={related} accent={accent} shareUrl={`${BASE}/actualites/${slug}`} />
      </div>
    </>
  );
}
